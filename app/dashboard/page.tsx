...
export default function DashboardPage() {
  ...
  // Load availability for both people
  useEffect(() => {
    const unsubs: (() => void)[] = []
  ...
  // Combined day status: best of ido+ofek
  function getDayColor(dk: string): 'green' | 'yellow' | 'red' | 'empty' {
    ...
    const both = [ido, ofek]
    if (both.includes('full'))    return 'green'
    if (both.includes('partial')) return 'yellow'
    const hasRequests = requests.filter(r => r.date === dk && r.status !== 'rejected').length > 0
    if (hasRequests) return 'red'
    return 'empty'
  }
  ...
  function getDayRequests(dk: string) {
    return requests.filter(r => r.date === dk)
  }
        ...
        <div className="flex-1">
          ...
          <div className="grid grid-cols-7 gap-1">
            ...
            {days.map(day => {
              ...
              const dayReqs = getDayRequests(dk)
                ...
                >
                  ...
                  <div className="flex gap-0.5 mt-0.5">
                    {pendingReqs.length > 0 && (
                      <span className="w-1.5 h-1.5 rounded-full bg-yellow-500" title="בקשה ממתינה" />
                    )}
                    {approvedReqs.length > 0 && (
                      <span className="w-1.5 h-1.5 rounded-full bg-blue-500" title="בקשה אושרה" />
                    )}
                  </div>
                </button>
              )
          ...
          <div className="flex gap-4 mt-4 text-xs text-gray-500 flex-wrap">
            ...
            <span className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-yellow-500 inline-block"/> בקשה ממתינה</span>
            <span className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-blue-500 inline-block"/> בקשה אושרה</span>
          </div>
        </div>
        ...
        {selectedDay && (
          <div className="w-80 self-start sticky top-6 space-y-4">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              ...
              {/* Request button */}
              {(getPersonDayStatus('ido', selectedDay) !== 'empty' ||
                getPersonDayStatus('ofek', selectedDay) !== 'empty') && (
                <button
                ...
                <div className="flex gap-2">
                  {(['ido', 'ofek'] as Person[]).map(p => {
                    ...
                    return (
                      <button
                        key={p}
                        onClick={() => !disabled && setSelectedPerson(p)}
                        disabled={disabled}
                      ...
                      >
                        ...
                        <span className={`block text-xs font-normal mt-0.5 ${
                          ...
                          status === 'full' ? 'text-emerald-600' : 'text-yellow-600'
                        }`}>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select value={startTime} onChange={e => setStartTime(e.target.value)}
                    ...
                    {getAvailableSlots(selectedPerson, selectedDay).map(h => (
                ...
                <div>
                  ...
                  <select value={endTime} onChange={e => setEndTime(e.target.value)}
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-500 text-right">
                    <option value="">בחר...</option>
                    {getAvailableSlots(selectedPerson, selectedDay)
